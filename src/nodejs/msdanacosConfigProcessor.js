 ...
 * @constructor
 */
function msdanacosConfigProcessor() {
}

msdanacosConfigProcessor.prototype.setModuleDependencies = function (options) {
    logger.info("setModuleDependencies called");
    configTaskUtil = options.configTaskUtil;
};

msdanacosConfigProcessor.prototype.WORKER_URI_PATH = "shared/iapp/processors/msdanacosConfig";

msdanacosConfigProcessor.prototype.onStart = function (success) {
    logger.fine("MSDA: OnStart, msdanacosConfigProcessor.prototype.onStart");
    ...
    try {
        fs.access(msdanacosOnPollingSignal, fs.constants.F_OK, function (err) {
            if (err) {
                logger.fine("MSDAnacos audit OnStart, the polling signal is off. ", err.message);
            } else {
                logger.fine("MSDA nacos audit onStart: ConfigProcessor started, clear the signal.");
                fs.unlinkSync(msdanacosOnPollingSignal);
            }
        });
    } catch(err) {
        logger.fine("MSDAnacos: OnStart, hits error while check pooling signal. ", err.message);
    }
    */
 ...
 */
msdanacosConfigProcessor.prototype.onPost = function (restOperation) {
    var configTaskState,
        blockState,
        oThis = this;
    ...
    try {
        ...
        logger.fine("MSDAnacos: onPost, hit error while set polling signal: ", error.message);
    }
    */
 ...
 * @param restOperation - originating rest operation that triggered this processor
 */
msdanacosConfigProcessor.prototype.onDelete = function (restOperation) {
    var configTaskState,
        blockState;
    var oThis = this;
