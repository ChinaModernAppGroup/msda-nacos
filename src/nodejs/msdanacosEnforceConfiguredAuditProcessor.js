...
function msdanacosEnforceConfiguredAuditProcessor() {
    // you can also use this.logger on a RestWorker
    // Using directly from require (below) is another way to call log events
    logger.info("loading msdanacos Enforce Configured Audit Processor");
}
...
msdanacosEnforceConfiguredAuditProcessor.prototype.WORKER_URI_PATH = "shared/iapp/processors/msdanacosEnforceConfiguredAudit";

msdanacosEnforceConfiguredAuditProcessor.prototype.onStart = function (success) {
    logger.fine("msdanacosEnforceConfiguredAuditProcessor.prototype.onStart");
    //logger.fine("MSDA nacos audit onStart: ConfigProcessor polling state: ");
...
msdanacosEnforceConfiguredAuditProcessor.prototype.onPost = function (restOperation) {
    ...
    try {
        ...
        */
        ...
        logger.fine('MSDA nacos Audit: msdaOnpolling: ', msdaOnPolling);
        fs.access(msdanacosOnPollingSignal, fs.constants.F_OK, function (err) {
            if (err) {
                logger.fine('MSDA nacos Audit: Checking polling signal hits error: ', err.message);
                logger.fine("MSDA nacos audit onPost: ConfigProcessor is NOT on polling state, will set msdaOnpolling status to FALSE.");
                msdaOnPolling = false;
                try {
                    ...
                    logger.fine("MSDA nacos audit onPost: trigger ConfigProcessor onPost ");
                } catch (err) {
                    logger.fine("MSDA nacos audit onPost: Failed to send out restOperation. ", err.message);
                }
            } else {
                logger.fine("MSDA nacos audit onPost: ConfigProcessor is on polling state, will set msdaOnPolling status to TRUE.");
                logger.fine("MSDA nacos audit onPost: ConfigProcessor is on polling state, no need to fire an onPost.");
                msdaOnPolling = true;
            }
        });
    } catch (ex) {
...
msdanacosEnforceConfiguredAuditProcessor.prototype.finishOperation = function( restOperation, auditTaskState ) {
    restOperation.setBody(auditTaskState);
    this.completeRestOperation(restOperation);
    logger.fine(getLogHeader() + "DONE" );
};

module.exports = msdanacosEnforceConfiguredAuditProcessor;
